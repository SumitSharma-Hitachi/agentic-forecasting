...
- **Copper forecasting (shipped)** — `implementations/copper_forecasting/` adds a cache-first FRED commodity workflow with naive and traditional baselines, AutoARIMA and Kalman forecasting, forecast combinations, three primary agent information designs, and a visual three-period backtest analysis. Its 2026 deep dive adds 24/36/60-month window sensitivity, an outcome-informed signal reinterpretation audit, and separate signal-disciplined news-plus-model-panel diagnostics on `gemini-3.1-flash-lite-preview` and `gemini-3.5-flash` that remain outside the primary leaderboard. The backtest notebook also parses and visualizes an uploaded 21-series Chinese NBS activity panel and checks cutoff-specific Pearson/Spearman relationships from six-month lags through six-month leads. A standalone selectable-cutoff runner asks the advanced revised agent only for the horizon-six month and supplies the matching row from a 36-month numerical panel. When at least 12 cutoff-safe NBS pairs exist, it also supplies 36 months of total fixed-asset growth and emphasizes the observed relationship as a high-weight directional prior using the cutoff-specific sign; older periods without sufficient NBS coverage retain the revised news-plus-model-panel workflow and explicitly omit that signal. It reports the resulting one-row MAE and writes a trend/error chart. Because the NBS file lacks release timestamps and the relationship was selected retrospectively, this remains an assumption-bound diagnostic pending vintage-aware rolling-origin lift over the price-only baseline.

### Agent and analyst depth

The four curriculum domain implementations (S&P 500, food, energy, BoC) ship a **`starter_agent/` module + `99_starter_agent.ipynb`** — a fresh, participant-owned agent template with toggleable proxy news search and E2B code execution, two lightweight tool-usage skills, an interactive (Track 2) cell, and one scored (Track 1) prediction. Copper instead keeps its three agent information designs in a compact notebook series. Natural next steps from here: richer E2B code-execution configs, prompt and context-formatting optimization, and deeper Track 2 interactive analyst configurations per use case (see [`../docs/adk-skills-guide.md`](../docs/adk-skills-guide.md) for the skill design rules).
